...
public class UIScrollbar extends CoreWidget {
    ...
    private final InteractionListener handleListener = new BaseInteractionListener() {
    ...
    private final InteractionListener sliderListener = new BaseInteractionListener() {
        @Override
        public boolean onMouseClick(NUIMouseClickEvent event) {
            if (event.getMouseButton() == MouseInput.MOUSE_LEFT) {
                Vector2i pos = event.getRelativeMousePosition();
